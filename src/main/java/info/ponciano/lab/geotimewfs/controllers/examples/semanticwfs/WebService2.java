...
public class WebService2 {
    ...
    /**
     * Constructor for this class. Loads configuration files and performs
     * initializations.
     *
     * @throws IOException on error
     */
    public WebService2() throws IOException {
        ...
        if (Conf.get().getWfsconf() == null) {
            try {
                System.out.println(new String(Files.readAllBytes(Paths.get("Conf.get().getWfsconf().json")), StandardCharsets.UTF_8));
                System.out.println(new JSONObject(new String(Files.readAllBytes(Paths.get("Conf.get().getWfsconf().json")), StandardCharsets.UTF_8)));
            } catch (Exception e) {
                e.printStackTrace();
            }
            System.out.println(Conf.get().getWfsconf());
            System.out.println("in");
        }
        System.out.println("out");
        for (Integer i = 0; i < Conf.get().getWfsconf().getJSONArray("datasets").length(); i++) {
            JSONObject featuretype = Conf.get().getWfsconf().getJSONArray("datasets").getJSONObject(i);
        ...
        htmlHead = "<html><head><link rel=\"stylesheet\" href=\"https://unpkg.com/leaflet@1.5.1/dist/leaflet.css\"\r\n"
                ...
                + "   crossorigin=\"\"/>\r\n" + "<script src=\"" + "/js"
                + "/config/js/proj4.js\"></script>" + "<script src=\"" + "/js"
                + "/config/js/prefixes.js\"></script>"
                + " <link rel=\"stylesheet\" href=\"" + "/js" + "/config/css/leaflet_legend.css\" />"
                ...
                + " <link rel=\"stylesheet\" href=\"" + "/js" + "/config/css/style.css\">"
                ...
                + "<script src=\"" + "/js" + "/config/js/leaflet.pattern.js\"></script>\r\n"
                + "<script src=\"" + "/js"
    ...
    public Response openapiJSON() {
        Object get = Conf.get().getWfsconf().get("baseurl");
        try {
            String result = onpenapiJSON2(get.toString());
    ...
    /**
     * Generates OpenAPI definitions of the SemanticWFS services for the use
    ...
    /**
     * Returns a style information in a given format for a given featuretype and
     * styleid.
     *
    ...
    public Response getCollectionStyle(
        ...
        for (int i = 0; i < Conf.get().getWfsconf().getJSONArray("datasets").length(); i++) {
            JSONObject curobj = Conf.get().getWfsconf().getJSONArray("datasets").getJSONObject(i);
    ...
    public Response getCollectionStyles(
        ...
        if (format.contains("json")) {
            return Response.ok(TripleStoreConnector.getStyleNames("/js", workingobj, format))
    ...
    public Response collections(
        ...
        if (format != null && format.contains("json")) {
            ...
            link.put("href", Conf.get().getWfsconf().get("baseurl") + "/collections?f=json");
            links.put(link);
            link = new JSONObject();
            link.put("rel", "alternate");
            link.put("title", "This document as XML");
            link.put("type", "text/xml");
            link.put("href", Conf.get().getWfsconf().get("baseurl") + "/collections?f=gml");
            links.put(link);
            link = new JSONObject();
            link.put("rel", "alternate");
            link.put("title", "This document as HTML");
            link.put("type", "text/html");
            link.put("href", Conf.get().getWfsconf().get("baseurl") + "/collections?f=html");
            links.put(link);
            link = new JSONObject();
            ...
            for (int i = 0; i < Conf.get().getWfsconf().getJSONArray("datasets").length(); i++) {
                ...
                for (ResultFormatter formatter : ResultFormatter.resultMap.values()) {
                    link = new JSONObject();
                    link.put("rel", "item");
                    link.put("href", "/js" + "/collections/" + curobj.getString("name")
                ...
                link.put("title", "This document");
                link.put("type", "application/json");
                link.put("href", Conf.get().getWfsconf().get("baseurl") + "/collections/" + curobj.getString("name") + "?f=json");
                colinks.put(link);
                link = new JSONObject();
                link.put("rel", "alternate");
                link.put("title", "This document as XML");
                link.put("type", "text/xml");
                link.put("href", Conf.get().getWfsconf().get("baseurl") + "/collections/" + curobj.getString("name") + "?f=xml");
                colinks.put(link);
                link = new JSONObject();
                link.put("rel", "alternate");
                link.put("title", "This document as HTML");
                link.put("type", "text/html");
                link.put("href", Conf.get().getWfsconf().get("baseurl") + "/collections/" + curobj.getString("name") + "?f=html");
                colinks.put(link);
                link = new JSONObject();
                link.put("rel", "describedBy");
                link.put("title", "XML Schema for this dataset");
            ...
            try {
                ...
                writer.writeCharacters(Conf.get().getWfsconf().getString("servicetitle"));
                writer.writeEndElement();
                ...
                writer.writeAttribute("href", Conf.get().getWfsconf().get("baseurl") + "/collections?f=xml");
                writer.writeEndElement();
                writer.writeStartElement("http://www.w3.org/2005/Atom", "link");
                writer.writeAttribute("rel", "alternate");
                writer.writeAttribute("title", "This document as JSON");
                writer.writeAttribute("type", "application/json");
                writer.writeAttribute("href", Conf.get().getWfsconf().get("baseurl") + "/collections?f=json");
                writer.writeEndElement();
                writer.writeStartElement("http://www.w3.org/2005/Atom", "link");
                writer.writeAttribute("rel", "alternate");
                writer.writeAttribute("title", "This document as HTML");
                writer.writeAttribute("type", "application/json");
                writer.writeAttribute("href", Conf.get().getWfsconf().get("baseurl") + "/collections?f=html");
                writer.writeEndElement();
                writer.writeStartElement("http://www.w3.org/2005/Atom", "link");
                ...
                for (int i = 0; i < Conf.get().getWfsconf().getJSONArray("datasets").length(); i++) {
                    ...
                    for (ResultFormatter formatter : ResultFormatter.resultMap.values()) {
                        ...
                        writer.writeAttribute("href", "/js" + "/collections/"
            ...
            builder.append("<a href=\"" + "/js" + "\">Landingpage</a> / <a href=\"" + "/js" + "/collections/\">Collections</a>");
            ...
            for (int i = 0; i < Conf.get().getWfsconf().getJSONArray("datasets").length(); i++) {
                ...
                builder.append("<td align=center><a href=\"" + "/js" + "/collections/"
                        + Conf.get().getWfsconf().getJSONArray("datasets").getJSONObject(i).get("name") + "?f=html\">"
                        + Conf.get().getWfsconf().getJSONArray("datasets").getJSONObject(i).get("name") + "</a></td><td align=center>");
                if (Conf.get().getWfsconf().getJSONArray("datasets").getJSONObject(i).has("description")) {
                    builder.append(Conf.get().getWfsconf().getJSONArray("datasets").getJSONObject(i).get("description"));
                }
                builder.append("</td><td align=center>");
                if (Conf.get().getWfsconf().getJSONArray("datasets").getJSONObject(i).has("schema")) {
                    builder.append("<a href=\"" + Conf.get().getWfsconf().getJSONArray("datasets").getJSONObject(i).get("schema")
                            + "\" target=\"_blank\">[Schema]</a>");
                } else {
                    builder.append("<a href=\"" + "/js" + "/collections/"
            ...
            builder.append("</div></div></div><footer id=\"footer\"><table width=100%><tbody><tr><td><a href=\"" + "/js"
                    + "/?f=html\">Back to LandingPage</a></td><td align=right>This page in <a href=\""
                    + "/js" + "/collections?f=gml\">[XML]</a> <a href=\""
                    + "/js"
    ...
    public Response getFeatureById(
        ...
        if (format != null && format.contains("json")) {
            ...
            for (ResultFormatter formatter : ResultFormatter.resultMap.values()) {
                ...
                link.put("href", "/js" + "/collections/" + collectionid + "/items/" + featureid
            ...
            builder.append("<div class=\"sticky row crumbs\"><div class=\"col-sm-12 col-md-10 col-md-offset-1\">");
            builder.append("<a href=\"" + "/js" + "\">Landingpage</a> / <a href=\"" + "/js" + "/collections/\">Collections</a> / <a href=\"" + "/js" + "/collections/" + collectionid + "\">" + workingobj.getString("name") + "</a> / <a href=\"" + "/js" + "/collections/" + collectionid + "/items?f=html\">Items</a> / <a href=\"" + "/js" + "/collections/" + collectionid + "/items/" + featureid + "?f=html\">" + featureid + "</a>");
            builder.append("</div></div>");
            builder.append("<div class=\"container-fluid\" role=\"main\"><div class=\"row\">");
            builder.append(res);
            builder.append("</div></div></div></div><footer id=\"footer\"><table width=100%><tbody><tr><td><a href=\"" + "/js" + "/collections/"
                    + collectionid + "?f=html\">Back to " + collectionid
                    + " Collection</a></td><td align=right>This page in <a href=\"" + "/js"
                    + "/collections/" + workingobj.getString("name") + "/items/" + featureid
                    + "?f=gml\">[GML]</a> <a href=\"" + "/js" + "/collections/"
    ...
    public Response queryables(@Parameter(description = "The id of the collection to be considered") @PathParam("collectionid") String collectionid,
        ...
        builder.append("</h1></header>");
        builder.append("<div class=\"sticky row crumbs\"><div class=\"col-sm-12 col-md-10 col-md-offset-1\">");
        builder.append("<a href=\"" + "/js" + "\">Landingpage</a> / <a href=\"" + "/js" + "/collections/\">Collections</a> / <a href=\"" + "/js" + "/collections/" + collectionid + "\">" + workingobj.getString("name") + "</a> / <a href=\"" + "/js" + "/collections/" + collectionid + "/queryables?f=html\">Queryables</a>");
        builder.append("</div></div>");
        builder.append("<div class=\"container-fluid\" role=\"main\"><div class=\"row\">");
        ...
        builder2.append("</table><br/>Styles:"
                + TripleStoreConnector.getStyleNames("/js", workingobj, format));
        builder.append("var geojson=" + geojson.toString() + "</script>");
        builder.append(builder2.toString());
        builder.append("</td></tr></table>");
        builder.append("</div></div><footer id=\"footer\"><table width=100%><tbody><tr><td><a href=\"" + "/js"
                + "/collections/" + workingobj.getString("name") + "?f=html\">Back to Collections</a></td><td align=right>This page in <a href=\""
                + "/js" + "/collections/" + workingobj.getString("name")
                + "/queryables?f=gml\">[GML]</a> <a href=\"" + "/js" + "/collections/"
    ...
    public Response landingPage(@Parameter(description = "The format of the landingpage") @DefaultValue("html") @QueryParam("f") String format) {
        if (format == null || format.contains("json")) {
            ...
            JSONArray links = new JSONArray();
            JSONObject link = new JSONObject();
            link.put("href", "/js" + "?f=json");
            link.put("rel", "self");
            link.put("type", "application/json");
            link.put("title", "This document");
            links.put(link);
            link = new JSONObject();
            link.put("href", "/js" + "?f=xml");
            link.put("rel", "alternate");
            link.put("type", "application/xml");
            link.put("title", "This document as XML");
            links.put(link);
            link = new JSONObject();
            link.put("href", "/js" + "?f=html");
            link.put("rel", "alternate");
            link.put("type", "text/html");
            link.put("title", "This document as HTML");
            links.put(link);
            link = new JSONObject();
            link.put("href", "/js" + "/openapi");
            ...
            links.put(link);
            link = new JSONObject();
            link.put("href", "/js" + "/api/");
            link.put("rel", "service-doc");
            link.put("type", "text/html");
            link.put("title", "The API definition (HTML)");
            links.put(link);
            link = new JSONObject();
            link.put("href", "/js" + "/conformance?f=html");
            link.put("rel", "conformance");
            link.put("type", "text/html");
            link.put("title", "Conformance Declaration as HTML");
            links.put(link);
            link = new JSONObject();
            link.put("href", "/js" + "/conformance?f=xml");
            link.put("rel", "conformance");
            link.put("type", "application/xml");
            link.put("title", "Conformance Declaration as XML");
            links.put(link);
            link = new JSONObject();
            link.put("href", "/js" + "/conformance?f=json");
            link.put("rel", "conformance");
            link.put("type", "application/json");
            link.put("title", "Conformance Declaration as JSON");
            links.put(link);
            link = new JSONObject();
            link.put("href", "/js" + "/collections?f=json");
            link.put("rel", "data");
            link.put("type", "application/json");
            link.put("title", "Collections Metadata as JSON");
            links.put(link);
            link = new JSONObject();
            link.put("href", "/js" + "/collections?f=xml");
            link.put("rel", "data");
            link.put("type", "application/xml");
            link.put("title", "Collections Metadata as XML");
            links.put(link);
            link = new JSONObject();
            link.put("href", "/js" + "/collections?f=html");
            link.put("rel", "data");
            link.put("type", "text/html");
            link.put("title", "Collections Metadata as HTML");
            links.put(link);
            result.put("title", Conf.get().getWfsconf().getString("servicetitle"));
            result.put("description", Conf.get().getWfsconf().getString("servicedescription"));
            result.put("links", links);
            JSONArray collections = new JSONArray();
            result.put("collections", collections);
            for (int i = 0; i < Conf.get().getWfsconf().getJSONArray("datasets").length(); i++) {
                ...
                link.put("title", "This document as XML");
                link.put("type", "application/xml");
                link.put("href", Conf.get().getWfsconf().get("baseurl") + "/collections/" + curobj.getString("name") + "?f=gml");
                colinks.put(link);
                link = new JSONObject();
                link.put("rel", "alternate");
                link.put("title", "This document as HTML");
            ...
            try {
                ...
                writer.writeEndElement();
                writer.writeStartElement("Description");
                writer.writeCharacters(Conf.get().getWfsconf().getString("servicedescription"));
                writer.writeEndElement();
                writer.writeStartElement("http://www.w3.org/2005/Atom", "link");
                writer.writeAttribute("rel", "self");
                writer.writeAttribute("title", "This document");
                writer.writeAttribute("type", "application/xml");
                writer.writeAttribute("href", "/js" + "/?f=gml");
                writer.writeEndElement();
                writer.writeStartElement("http://www.w3.org/2005/Atom", "link");
                writer.writeAttribute("rel", "alternate");
                writer.writeAttribute("title", "This document as JSON");
                writer.writeAttribute("type", "application/json");
                writer.writeAttribute("href", "/js" + "/?f=json");
                writer.writeEndElement();
                writer.writeStartElement("http://www.w3.org/2005/Atom", "link");
                writer.writeAttribute("rel", "alternate");
                writer.writeAttribute("title", "This document as HTML");
                writer.writeAttribute("type", "text/html");
                writer.writeAttribute("href", "/js" + "/?f=html");
                writer.writeEndElement();
                writer.writeStartElement("http://www.w3.org/2005/Atom", "link");
                writer.writeAttribute("rel", "service-doc");
                writer.writeAttribute("title", "The API definition (HTML)");
                writer.writeAttribute("type", "text/html");
                writer.writeAttribute("href", "/js" + "/api/");
                writer.writeEndElement();
                writer.writeStartElement("http://www.w3.org/2005/Atom", "link");
                ...
                writer.writeAttribute("href", "/js" + "/openapi.yaml");
                writer.writeEndElement();
                writer.writeStartElement("http://www.w3.org/2005/Atom", "link");
                writer.writeAttribute("rel", "service-desc");
            ...
            StringBuilder builder = landingPageHTMLRaw("servicedescription");
    ...
    public static StringBuilder landingPageHTMLRaw(String servicedescription) throws JSONException {
        String servicetitle = "servicetitle";

        StringBuilder builder = new StringBuilder();
        builder.append("<html><head><link rel=\"stylesheet\" href=\"https://unpkg.com/leaflet@1.5.1/dist/leaflet.css\"\r\n"
                + "   integrity=\"sha512-xwE/Az9zrjBIphAcBb3F6JVqxf46+CDLwfLMHloNu6KEQCAWi6HcDUbeOfBIptF7tcCzusKFjFw2yuvEpDL9wQ==\"\r\n"
                + "   crossorigin=\"\"/>\r\n" + "<script src=\"" + "/js"
                + "/config/js/proj4.js\"></script>" + "<script src=\"" + "/js"
                + "/config/js/prefixes.js\"></script>"
                ...
                + "<script src='https://api.mapbox.com/mapbox.js/plugins/leaflet-fullscreen/v1.0.1/Leaflet.fullscreen.min.js'></script>\r\n"
                + "<link href='https://api.mapbox.com/mapbox.js/plugins/leaflet-fullscreen/v1.0.1/leaflet.fullscreen.css' rel='stylesheet' /></head>");
        builder.append("<body><header id=\"header\"><h1 align=\"center\">LandingPage: " + servicetitle);
        builder.append("</h1></header>");
        builder.append("<div class=\"sticky row crumbs\"><div class=\"col-sm-12 col-md-10 col-md-offset-1\">");
        builder.append("<a href=\"" + "/js" + "\">Landingpage</a>");
        builder.append("</div></div>");
        builder.append("<div class=\"container-fluid\" role=\"main\"><div class=\"row\"><div class=\"col-sm-12\"><p>"
                + servicedescription + "</p><ul>");
        builder.append("<li>API Documentation in <a href=\"" + "/js"
                + "/api/\">[HTML]</a>");
        builder.append(" <a href=\"" + "/js" + "/openapi.yaml\">[YAML]</a>");
        builder.append(" <a href=\"" + "/js" + "/openapi.json\">[JSON]</a></li>");
        builder.append("<li>Conformance Declaration in <a href=\"" + "/js"
                + "/conformance?f=html\">[HTML]</a>");
        builder.append(" <a href=\"" + "/js" + "/conformance?f=gml\">[XML]</a>");
        builder.append(" <a href=\"" + "/js" + "/conformance?f=json\">[JSON]</a></li>");
        builder.append("<li>Collections Metadata in <a href=\"" + "/js"
                + "/collections?f=html\">[HTML]</a>");
        builder.append(" <a href=\"" + "/js" + "/collections?f=gml\">[XML]</a>");
        builder.append(" <a href=\"" + "/js" + "/collections?f=json\">[JSON]</a></li></ul>");
        ...
        builder.append(
                "</li></ul>Local Options:<ul><li><a href=\"" + "/js" + "/config/geotreeview.html\">Ontology Browser</a></li>"
                + "<li><a href=\"" + "/js" + "/config/queryinterface.html\">SPARQL Query Interface</a></li>"
                + "<li><a href=\"" + "/js"
                + "/config/configuration.html\">Semantic WFS Configuration</a></li><li><a href=\"../importer/\">Semantic Uplift Tools</a></li></ul></div></div></div><footer id=\"footer\"><table width=100%><tbody><tr><td><a href=\"" + "/js"
                + "/?f=html\">Back to LandingPage</a></td><td align=right>This page in <a href=\""
                + "/js" + "/?f=xml\">[XML]</a> <a href=\""
                + "/js"
     ...
     * @param collectioncall indicates whether this method has been called from
    ...
    public Response getCollectionMetadata(@PathParam("collectionid") String collectionid,
        ...
        String collectionurl = "/js" + "/collections/" + collectionid;
        try {
    ...
    public Response collectionInformation(
        ...
        if (format != null && format.contains("json")) {
            ...
            result.put("crs", crs);
            JSONObject link = new JSONObject();
            link.put("href", "/js" + "/collections/" + collectionid + "?f=json");
            link.put("rel", "self");
            link.put("type", "application/json");
            link.put("title", "This document");
            resultlinks.put(link);
            link = new JSONObject();
            link.put("href", "/js" + "/collections/" + collectionid + "?f=xml");
            link.put("rel", "alternate");
            link.put("type", "application/xml");
            link.put("title", "This document as XML");
            resultlinks.put(link);
            link = new JSONObject();
            link.put("href", "/js" + "/collections/" + collectionid + "?f=html");
            link.put("rel", "alternate");
            link.put("type", "text/html");
            link.put("title", "This document as HTML");
            resultlinks.put(link);
            ...
            for (ResultFormatter formatter : ResultFormatter.resultMap.values()) {
                ...
                link.put("href", "/js" + "/collections/" + collectionid + "/items" + "?f="
                        + formatter.urlformat);
                link.put("type", formatter.exposedType);
            ...
            link = new JSONObject();
            link.put("rel", "describedBy");
            link.put("href", "/js" + "/collections/" + collectionid + "/schema/");
            link.put("type", "application/xml");
            ...
            try {
                ...
                writer.writeAttribute("title", "This document");
                writer.writeAttribute("type", "application/xml");
                writer.writeAttribute("href", "/js" + "/collections/" + collectionid + "/?f=gml");
                writer.writeEndElement();
                writer.writeStartElement("http://www.w3.org/2005/Atom", "link");
                writer.writeAttribute("rel", "alternate");
                writer.writeAttribute("title", "This document as JSON");
                writer.writeAttribute("type", "application/json");
                writer.writeAttribute("href", "/js" + "/collections/" + collectionid + "/?f=json");
                writer.writeEndElement();
                writer.writeStartElement("http://www.w3.org/2005/Atom", "link");
                writer.writeAttribute("rel", "alternate");
                writer.writeAttribute("title", "This document as HTML");
                writer.writeAttribute("type", "text/html");
                writer.writeAttribute("href", "/js" + "/collections/" + collectionid + "/?f=html");
                writer.writeEndElement();
                writer.writeEndElement();
                writer.writeStartElement("collections");
                ...
                writer.writeAttribute("href",
                        "/js" + "/collections/" + workingobj.getString("name") + "/schema/");
                writer.writeEndElement();
                /*writer.writeStartElement("Extent");
				writer.writeStartElement("Spatial");
				writer.writeAttribute("crs", "http://www.opengis.net/def/crs/OGC/1.3/CRS84");
            ...
            builder.append("</h1></header>");
            builder.append("<div class=\"sticky row crumbs\"><div class=\"col-sm-12 col-md-10 col-md-offset-1\">");
            builder.append("<a href=\"" + "/js" + "\">Landingpage</a> / <a href=\"" + "/js" + "/collections/\">Collections</a> / <a href=\"" + "/js" + "/collections/" + collectionid + "\">" + workingobj.getString("name") + "</a>");
            builder.append("</div></div>");
            ...
            builder.append("</a>");
            builder.append("<br/>CRS:<select id=\"crs\"></select><button id=\"showfeaturebutton\" onclick=\"showCollections('" + "/js" + "/collections/" + workingobj.getString("name") + "/items')\"/>Show</button></section></div></div></div>");
            builder.append("<footer id=\"footer\"><table width=100%><tbody><tr><td><a href=\"" + "/js"
     ...
     * @param bbox The bounding box in which features which should be returned
     * are contained
            ...
            if (format != null && format.contains("json") && !format.contains("covjson") && !format.contains("jsonp") && !format.contains("seq") && !format.equalsIgnoreCase("rdfjson")) {
                ...
                for (ResultFormatter formatter : ResultFormatter.resultMap.values()) {
                    ...
                    if (formatter.exposedType.contains("geojson")) {
                        ...
                        nextlink.put("href", "/js" + "/collections/" + collectionid + "/items?offset=" + (Integer.valueOf(offset) + Integer.valueOf(limit)) + "&limit=" + limit + "&f="
                                + formatter.urlformat);
                ...
                try {
                    ...
                    writer.writeAttribute("xmlns:gml", "http://www.opengis.net/gml/3.2");
                    writer.setDefaultNamespace("http://www.opengis.net/ogcapi-features-1/1.0");
                    for (String ns : WebService2.nameSpaceCache.get(collectionid.toLowerCase()).keySet()) {
                        writer.writeAttribute("xmlns:" + WebService2.nameSpaceCache.get(collectionid.toLowerCase()).get(ns), ns);
                    }
                    writer.setPrefix("atom", "http://www.w3.org/2005/Atom");
                    writer.writeAttribute("service", "OGCAPI-FEATURES");
                    writer.writeAttribute("version", "1.0.0");
                ...
                builder.append("</h1></header>");
                builder.append("<div class=\"sticky row crumbs\"><div class=\"col-sm-12 col-md-10 col-md-offset-1\">");
                builder.append("<a href=\"" + "/js" + "\">Landingpage</a> / <a href=\"" + "/js" + "/collections/\">Collections</a> / <a href=\"" + "/js" + "/collections/" + collectionid + "\">" + workingobj.getString("name") + "</a> / <a href=\"" + "/js" + "/collections/" + collectionid + "/items?f=html\">Items</a>");
                builder.append("</div></div>");
                builder.append("<div class=\"container-fluid\" role=\"main\"><div class=\"row\">");
                builder.append(res);
    ...
    public Response conformance(@Parameter(description = "The format of the conformance page") @DefaultValue("html") @QueryParam("f") String format) {
        if (format != null && format.contains("json")) {
            ...
            builder.append("</ul></div></div></div><footer id=\"footer\"><table width=\"100%\"><tbody><tr><td><a href=\"" + "/js"
                    + "/?f=html\">Back to LandingPage</a></td><td align=right>This page in <a href=\""
                    + "/js" + "/conformance?f=gml\">[GML]</a> <a href=\"" + "/js"
    ...
    public Response constructCapabilitiesWFS10(String version, String versionnamespace) throws XMLStreamException {
        ...
        writer.writeCharacters(Conf.get().getWfsconf().has("servicetitle") ? Conf.get().getWfsconf().getString("servicetitle") : "");
        writer.writeEndElement();
        writer.writeStartElement("Title");
        writer.writeCharacters(Conf.get().getWfsconf().has("servicetitle") ? Conf.get().getWfsconf().getString("servicetitle") : "");
        writer.writeEndElement();
        writer.writeStartElement("Abstract");
        writer.writeCharacters(Conf.get().getWfsconf().has("abstract") ? Conf.get().getWfsconf().getString("abstract") : "");
        writer.writeEndElement();
        writer.writeStartElement("Fees");
        writer.writeCharacters(Conf.get().getWfsconf().has("fees") ? Conf.get().getWfsconf().getString("fees") : "none");
        writer.writeEndElement();
        writer.writeStartElement("OnlineResource");
        writer.writeCharacters(Conf.get().getWfsconf().has("baseurl") ? "/js" : "");
        writer.writeEndElement();
        writer.writeEndElement();
        ...
        writer.writeAttribute("onlineResource", "/js" + "/wfs?");
        writer.writeEndElement();
        ...
        writer.writeEndElement();
        for (int i = 0; i < Conf.get().getWfsconf().getJSONArray("datasets").length(); i++) {
            describeFeatureTypeWFS10(writer, Conf.get().getWfsconf().getJSONArray("datasets").getJSONObject(i), versionnamespace,
                    version);
        }
        writer.writeEndElement();
    ...
    public Response constructCapabilitiesCSW(String version, String versionnamespace) throws XMLStreamException {
        ...
        writer.writeCharacters(
                Conf.get().getWfsconf().has("servicetitle") ? Conf.get().getWfsconf().getString("servicetitle").replace("WFS", "CSW") : "");
        writer.writeEndElement();
        ...
        writer.writeAttribute("xlink:href", "/js" + "/csw?");
        writer.writeEndElement();
    ...
    public Response constructCapabilities(String version, String versionnamespace) throws XMLStreamException {
        ...
        writer.writeStartElement(owsns, "Get");
        writer.writeAttribute("xlink:type", "simple");
        writer.writeAttribute("xlink:href", "/js" + "/wfs?");
        writer.writeEndElement();
        writer.writeStartElement(owsns, "Post");
        writer.writeAttribute("xlink:type", "simple");
        writer.writeAttribute("xlink:href", "/js" + "/wfs");
        ...
        writer.writeEndElement();
        for (int i = 0; i < Conf.get().getWfsconf().getJSONArray("datasets").length(); i++) {
            describeFeatureType(writer, Conf.get().getWfsconf().getJSONArray("datasets").getJSONObject(i), versionnamespace, version);
        }
        writer.writeEndElement();
        writer.writeStartElement("http://www.opengis.net/fes" + versionnamespace, "Filter_Capabilities");
        describeConformance(writer, versionnamespace, "http://www.opengis.net/fes" + versionnamespace);
    ...
    public Response describeFeatureType(
        ...
        // writer.writeAttribute("targetNamespace",(workingobj.has("namespace")?workingobj.getString("namespace"):"/js"));
        writer.writeDefaultNamespace("http://www.w3.org/2001/XMLSchema");
        writer.writeNamespace("app",
                (workingobj.has("namespace") ? workingobj.getString("namespace") : "/js"));
    ...
    public Response describeFeatureTypeJSON(
        ...
        schema.put("$id", "/js" + "/collections/" + typename + "/schema?f=json");
        ...
        for (String elem : mapping.keySet()) {
            ...
            prop.put("$id", "/js" + "/collections/" + typename + "/" + curprop);
            if (mapping.get(elem).contains("^^")) {
    ...
    public Response getFeature(@QueryParam("typename") String typename,
        ...
        if (output.contains("gml")) {
            ...
            try {
                ...
                writer.writeDefaultNamespace(("/js" + "/").replace("//", "/"));
                writer.writeNamespace("ows", "http://www.opengis.net/ows/1.1");
                writer.writeNamespace("sf", "http://www.opengis.net/ogcapi-features-1/1.0/sf");
                writer.writeNamespace("ogc", "http://www.opengis.net/ogc");
                writer.writeNamespace("gml", "http://www.opengis.net/gml");
    ...
    public Response getPropertyValue(
        ...
        if (output != null && output.contains("json")) {
            ...
            builder.append(res);
            builder.append("</div></div></div><footer id=\"footer\"><a href=\"" + "/js"
    ...
    public Boolean addFeatureType(
        ...
        if (true || name != null && !name.isEmpty() && user != null && (user.getUserlevel() == UserType.Configurer || user.getUserlevel() == UserType.Administrator)) {
            JSONArray datasets = Conf.get().getWfsconf().getJSONArray("datasets");
            System.out.println(Conf.get().getWfsconf());
    ...
    public Boolean saveFeatureTypes(
        ...
        if (user != null && (user.getUserlevel() == UserType.Configurer || user.getUserlevel() == UserType.Administrator)) {
            JSONArray datasets = Conf.get().getWfsconf().getJSONArray("featjson");
            Conf.get().getWfsconf().put("datasets", datasets);
            return true;
        }
        return false;
    }
    ...
    public String getFeatureTypes() {
        return Conf.get().getWfsconf().toString(2);
    }
    ...
    public Response queryConfigs() {
        ...
        for (int i = 0; i < Conf.get().getWfsconf().getJSONArray("datasets").length(); i++) {
            JSONObject curjson = Conf.get().getWfsconf().getJSONArray("datasets").getJSONObject(i);
     ...
     * @param query The query to send
     * @param endpoint The endpoint to query
     * @param geojson indicates whether to return an additional representation
     * as GeoJSON for a map view
     * @return The query result
     */
    @GET
    @Produces(MediaType.TEXT_PLAIN)
